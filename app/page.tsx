    ...
    <div className="h-screen flex flex-col bg-gray-100">
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-16 bg-white border-r flex flex-col items-center py-4 gap-4">
          <label className="text-sm cursor-pointer">上传<input type="file" accept="image/*" multiple onChange={handleUpload} className="hidden" /></label>
        ...
        <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} className={`flex-1 flex flex-col overflow-hidden transition ${dragging ? 'bg-blue-50 border-2 border-dashed border-blue-500' : 'bg-gray-50'}`}>
          <div onWheel={handleWheel} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={stopPanning} onMouseLeave={stopPanning} onDoubleClick={resetViewer} className={`relative flex-1 overflow-hidden bg-gray-50 select-none ${selectedImage ? (isPanning ? 'cursor-grabbing' : 'cursor-grab') : ''}`}>
            {selectedImage ? (
              <>
                <div className="absolute inset-0 flex items-center justify-center overflow-hidden">
                  <img src={selectedImage} alt="preview" draggable={false} className="bg-white border shadow pointer-events-none" style={{
                    maxWidth: 'none', maxHeight: 'none', width: 'auto', height: 'auto',
                    transform: `translate3d(${position.x}px, ${position.y}px, 0) scale(${scale})`,
                    transformOrigin: 'center center', willChange: 'transform',
                  }} />
                </div>
                <div className="absolute right-4 top-4 bg-black/70 text-white text-xs px-3 py-1 rounded-full pointer-events-none">{Math.round(scale * 100)}%</div>
                <div className="absolute left-4 bottom-4 bg-black/60 text-white text-xs px-3 py-2 rounded-lg pointer-events-none">滚轮缩放 · 拖动查看 · 双击重置</div>
              </>
            ) : (
              <div className="h-full flex items-center justify-center text-center text-gray-400">
                <div><div className="text-lg mb-2">拖拽图片到这里上传</div><div className="text-sm">最多 5 张 · 单张 ≤20MB</div></div>
              </div>
            )}
          </div>
        ...
        <div className="w-80 bg-white border-l p-5">
          ...
          <label className="block w-full bg-gray-900 text-white text-center py-3 rounded-lg cursor-pointer">上传图片<input type="file" accept="image/*" multiple onChange={handleUpload} className="hidden" /></label>
          ...
          <button onClick={handleConvert} disabled={loading} className="mt-6 w-full bg-blue-600 disabled:bg-gray-400 text-white py-3 rounded-lg">
            {loading ? '转换中...' : '开始转换'}
          </button>
        </div>
      </div>
